...
const checkDogs = function (dogsJulia, dogsKate) {
  const dosgJuliaUpdated = dogsJulia.slice(); // Makes a shallow copy
  dosgJuliaUpdated.splice(0, 1); // Remove first
  dosgJuliaUpdated.splice(-2); // Remove last two
  const dogsJuliaAndKate = dosgJuliaUpdated.concat(dogsKate);
  dogsJuliaAndKate.forEach((element, index) => {
    element >= 3
      ? console.log(
